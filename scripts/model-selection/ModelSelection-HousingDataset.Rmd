...
```{r Installing Libraries, results='hide', message=F, warning=F, cache = TRUE}
...
```{r Multiple Linear Regression Model, cache=TRUE}
...
```{r Anderson Darling normality test for testing residual error, cache=TRUE}
...
```{r Plotting the residuals, cache=TRUE}
...
```{r Box Cox transformation, cache=TRUE}
...
Let's compare the two models so far, the *stepwise* and *box-cox* models.

```{r Comparing the models, cache=TRUE, warning=FALSE}
...
```{r Final Anderson Darling test, cache=TRUE}
...
```{r tree error metrics, cache=TRUE, include=FALSE}
...
```{r Visualising the tree, cache=TRUE}
...
```{r getting the best hyperparameters, cache=TRUE}
...
```{r Variable Importance, cache=TRUE}
vip::vip(tuned_tree)
```
